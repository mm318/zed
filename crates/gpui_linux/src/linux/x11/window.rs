...
impl PlatformWindow for X11Window {
    ...
    fn draw(&self, scene: &Scene) {
        ...
        if inner.renderer.device_lost() {
            ...
            let display = rwh::HasDisplayHandle::display_handle(&raw_window)
                .expect("display handle should be available")
                .as_raw();
            let window = rwh::HasWindowHandle::window_handle(&raw_window)
                .expect("window handle should be available")
                .as_raw();
            inner
                .renderer
                .recover(display, window)
                .unwrap_or_else(|err| {
                    panic!(
