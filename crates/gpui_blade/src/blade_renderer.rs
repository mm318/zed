...
/// Configuration for a Blade rendering surface.
pub struct BladeSurfaceConfig {
    /// The size of the surface in device pixels.
    pub size: gpu::Extent,
    /// Whether the surface supports transparency.
...
/// GPU renderer backed by the Blade graphics library.
...
impl BladeRenderer {
    /// Creates a new Blade renderer for the given window and surface configuration.
    ...
    /// Updates the renderer's drawable size if it has changed.
    ...
    /// Updates the surface transparency setting, recreating pipelines if needed.
    ...
    /// Returns the platform atlas as a trait object.
    pub fn platform_atlas(&self) -> Arc<dyn PlatformAtlas> {
        self.atlas.clone()
    }
    ...
    // TODO: Implement actual GPU device loss detection once blade-graphics supports it.
    #[cfg(target_os = "linux")]
    pub fn device_lost(&self) -> bool {
        false
    }

    /// Attempts to recover from a GPU device loss.
    // TODO: Implement actual GPU device recovery once blade-graphics supports it.
    #[cfg(target_os = "linux")]
    pub fn recover(
        &mut self,
        ...
        anyhow::bail!("GPU device recovery is not yet supported by blade-graphics")
    }

    /// Returns the maximum supported texture dimension.
    // TODO: Query actual GPU max texture dimension once blade-graphics exposes it.
    #[cfg(target_os = "linux")]
    pub fn max_texture_size(&self) -> u32 {
        8192
    }

    /// Returns the Metal layer backing this renderer.
    #[cfg(target_os = "macos")]
    pub fn layer(&self) -> metal::MetalLayer {
        unsafe { foreign_types::ForeignType::from_ptr(self.layer_ptr()) }
    }

    /// Returns a raw pointer to the CAMetalLayer.
    #[cfg(target_os = "macos")]
    ...
    /// Destroys all GPU resources owned by this renderer.
    ...
    /// Draws the given scene to the surface.
